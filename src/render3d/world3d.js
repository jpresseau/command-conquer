...
/* One tree: trunk and three or four foliage tiers, sized and leaned by hash so a forest is
   a population rather than an array. Heights run 5-9 world units - a cell and a bit to two
   cells - which is the reference's tree-to-tank proportion. */
        ...
        for (var tx = x0; tx < x1; tx++) {
          ...
          if (k === RTS_T_TREE) {
            ...
            for (var tn = 0; tn < nt; tn++) {
              var ox = (_sprHash(tn, tx, 401) - 0.5) * (RTS_TILE - 1.6);
              var oz = (_sprHash(tz, tn, 409) - 0.5) * (RTS_TILE - 1.6);
              _r3dTree(faces, wx + ox, wz + oz,
                       _sprHash(tx + tn, tz, 419), _sprHash(tz, tx + tn, 421), _sprHash(tn, tx + tz, 431));
            }
            ...
            for (var rb = 0; rb < 3; rb++) {
              var rh = 2.2 + _sprHash(tx + rb, tz, 347) * 3.4;
              var rw = 1.6 + _sprHash(tz, tx + rb, 349) * 2.2;
              _r3Box(faces, wx + (_sprHash(rb, tx, 353) - 0.5) * 2.4, rb * 0.5,
                     wz + (_sprHash(rb, tz, 359) - 0.5) * 2.4,
                     rw, rh, rw * (0.8 + h1 * 0.5),
                     rb & 1 ? '#6a6f66' : '#7e837a', rb & 1 ? '#575b54' : '#8d9289');
            }
            for (var cg = 0; cg < 2; cg++) {
              var gh = 3 + _sprHash(tx, tz + cg, 433) * 3;
              _r3Cone(faces, wx + (_sprHash(cg, tx, 439) - 0.5) * 3,
                      0, wz + (_sprHash(cg, tz, 443) - 0.5) * 3,
                      0.9 + _sprHash(tx + cg, tz, 449) * 0.6, 0.1, gh,
            ...
            if (h1 < R3D_TUFT_ODDS) {
              for (var tf = 0; tf < R3D_TUFTS_PER_CELL; tf++) {
                var fx = wx + (_sprHash(tf, tx, 367) - 0.5) * RTS_TILE;
                var fz = wz + (_sprHash(tz, tf, 373) - 0.5) * RTS_TILE;
                var fh = 0.5 + _sprHash(tf + tx, tz, 379) * 0.7;
...
/* The crystals over the ore, rebuilt when the field actually changes. The stain in the
   terrain bake says WHERE ore is; these say it is a mineral, standing proud enough to catch
   the light. Height scales with what is left in the cell, so a field visibly wears down as
   the harvesters work it. */
  ...
  for (var tz = 0; tz < N; tz++) {
    for (var tx = 0; tx < N; tx++) {
      ...
      for (var c = 0; c < R3D_CRYSTALS_PER_CELL; c++) {
        var cx = wx + (_sprHash(c, tx, 383) - 0.5) * RTS_TILE;
        var cz = wz + (_sprHash(tz, c, 389) - 0.5) * RTS_TILE;
        var ch = (0.5 + _sprHash(c + tx, tz, 397) * 1.1) * (0.35 + frac);
