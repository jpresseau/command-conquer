/* ui/sidebar.js - the sidebar: tabs, the build list, and the per-frame readouts.
   Part of rts.ui, which owns the DOM.

   The superweapon buttons are ui/superbar.js. They live in the same strip and share the armed-
   cursor rule with REPAIR and SELL - whichever of the three is armed takes the next click on
   the map - so the two files call into each other's disarm; that handshake is the only coupling
   between them, and it is written down at both ends. */
