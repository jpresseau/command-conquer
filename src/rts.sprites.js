...
function _sprBuilding(key, side) {
  ...
  var C = RTS_PAL.conc, S = RTS_PAL.steel, DK = RTS_PAL.dark;
  var m = [], i;
  ...
  if (key === 'yard') {
    ...
    _r3Box(m, 0, 0, 2, W - 8, 25, D - 10, C[0]);
    _r3Box(m, 0, 25, -14, W - 18, 3, 16, TM[0], TM[1]);            /* team panel */
    for (i = 0; i < 4; i++) _r3Box(m, -24 + i * 16, 25, 6, 9, 5, 8, DK[1], DK[3]);  /* AC units */
    _r3Box(m, 0, 25, 22, W - 22, 2, 5, S[0], S[3]);                /* conduit run */
    _r3Box(m, 20, 25, 16, 16, 6, 12, S[2], S[1]);                  /* rooftop tank */
    ...
    _r3Box(m, 0, 0, 3, W - 8, 18, D - 12, C[0]);
    _r3Box(m, 0, 18, -3, W - 18, 3, 12, TM[0], TM[1]);
    ...
    _r3Box(m, -12, 0, -4, W - 34, 23, D - 22, C[0]);
    _r3Box(m, -12, 23, -16, W - 44, 3, 12, TM[0], TM[1]);
    for (i = 0; i < 3; i++) _r3Box(m, -26 + i * 13, 23, 2, 8, 5, 8, DK[1], DK[3]);
    ...
    _r3Box(m, 0, 0, 0, W - 8, 13, D - 10, C[0]);
    _r3Gable(m, 0, 13, 0, W - 4, 15, D - 6, RTS_PAL.dirt[1]);
    ...
    _r3Box(m, 0, 0, 0, W - 8, 17, D - 8, C[0]);
    for (i = 0; i < 9; i++) {                                      /* barrel roof segments */
      var a0 = (i / 9) * Math.PI, a1 = ((i + 1) / 9) * Math.PI;
      var r = (D - 10) / 2, hy = 17;
      var z0 = -Math.cos(a0) * r, y0 = hy + Math.sin(a0) * 9;
      var z1 = -Math.cos(a1) * r, y1 = hy + Math.sin(a1) * 9;
      /* Wound so the normal points up and outward. The other order aims it at the ground
         and the whole roof vanishes behind its own culled faces. */
      _r3F(m, [[(W - 8) / 2, y0, z0], [-(W - 8) / 2, y0, z0],
               [-(W - 8) / 2, y1, z1], [(W - 8) / 2, y1, z1]], i === 4 ? TM[0] : S[0]);
    }
    _r3Box(m, 0, 0, D / 2 - 3, 30, 15, 3, DK[1], DK[3]);           /* roll-up door */
    _r3Box(m, 0, 0, D / 2 + 1, 38, 1, 6, RTS_PAL.hazard[0], RTS_PAL.hazard[0]);
    _r3Cyl(m, -W / 2 + 10, 17, -12, 4, 18, S[0], DK[1], 10);        /* exhausts */
    _r3Cyl(m, -W / 2 + 19, 17, -12, 4, 13, S[0], DK[1], 10);
    ...
    _r3Cyl(m, 0, 0, 0, 11, 4, C[0], C[1], 16);
