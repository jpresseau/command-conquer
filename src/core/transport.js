...
/* UNIT.CPP Try_To_Deploy. An MCV becomes a Command Yard, and three details from the original
   ...
   - the yard is CENTRED ON the vehicle, from `Adjacent_Cell(Coord, FACING_NW)`: one step
     north-west is the footprint's origin, which for a 3x3 puts the MCV in the middle cell.
     This said "adjacent to the vehicle, not under it" for a long time and the arithmetic said
     something else again - it subtracted w-1, two cells for a 3x3, which made the vehicle's
     cell the SOUTH-EAST CORNER and grew the whole yard up and to the left. Nobody had measured
     it; the comment, docs/core-units.md and the answer given to a player all repeated the same
     unchecked sentence. The cost was not cosmetic: you park where you want the base, and the
     building lands two cells off in a direction nothing told you about - so a blocked patch
     up-left refuses the deploy while the ground you are looking at is clear;
...
function _rtsDeploy(e) {
  ...
  if (!sd) return false;
  /* One step north-west, as Adjacent_Cell gives it - NOT w-1, which is two for a 3x3. An even
     footprint has no middle cell, so the halving rounds down and the vehicle sits just past
     centre rather than at a corner; only the 3x3 yard deploys today, where it is exact. */
  var tx = _rtsTX(e.x) - ((sd.w - 1) >> 1), tz = _rtsTX(e.z) - ((sd.h - 1) >> 1);
