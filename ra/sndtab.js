/* What each sound in Red Alert's archives IS, keyed by its index hash.
   ...
   never recovered; the hash makes them unnecessary.

   THIS IS A CATALOGUE OF THE ARCHIVES, NOT A LIST OF WHAT THE GAME PLAYS. 120 of these are
   reachable; the ~32 that are not are voices for units this game does not field - the
   Mechanic, the Shock Trooper, the Spy, the Tesla coil, the Chronosphere, the mine layer.
   They are kept because the identification is a fact about the player's files and stays true
   whether or not anything asks for it, and because a unit that arrives later should not have
   to be identified again. Mapping them onto whichever existing unit is nearest would be
   worse than leaving them: a thief is not a spy, and hearing the wrong voice is a bug that
   sounds like a feature. Sounds RA does not have (the dozen
...
(function (exp) {
  ...
  /* The browser gets the TABLE, not a wrapper around it - every consumer indexes
     window.RA_SNDTAB directly. CommonJS gets the wrapper because require() callers
     destructure it. The two are deliberately different shapes. */
  else if (typeof window !== 'undefined') window.RA_SNDTAB = exp.RA_SNDTAB;
})({ RA_SNDTAB: RA_SNDTAB });
